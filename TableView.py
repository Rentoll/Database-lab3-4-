...
class TableDisplay:
    ...
    def create_extra_window(self, function, columns: list = None, fill_entry=False):
        ...
        button = Button(extra_window, text="OK", width=40,
                        command=lambda: [update_tree(), extra_window.destroy()])
    ...
    def show(self):
        add_button = Button(self.frame_buttons, text='Добавить запись',
                            command=lambda: self.create_function_window(self.Operations.ADD))
        add_button.pack(side=LEFT)

        update_button = Button(self.frame_buttons, text='Обновить запись',
                               command=lambda: self.create_function_window(self.Operations.UPDATE))
        update_button.pack(side=LEFT)

        delete_by_id_button = Button(self.frame_buttons, text='Удалить запись по id',
        ...
        delete_by_address_button = Button(self.frame_buttons, text='Удалить запись по адресу',
        ...
        search_by_address_button = Button(self.frame_buttons, text='Поиск записи по адресу',
                                         state='disabled' if self.table.__class__.__name__ == "Table" else 'normal',
